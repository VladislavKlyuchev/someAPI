...
module.exports = function (app, passport) {
    ...
    app.post('/listChannels', validSessionId, (req, res) => {
        req.db.user.findOne({ where: { id: req.userSession.userId } }).then((user) => {
            req.db.chPackages.findAll({
                where: { packageId: user.packageId }, include: [
                    {
                        model: req.db.channels,
                        attributes: ['channelId', 'channelName', 'logoPath', 'streamPath', 'timeshift', 'hidden']
                    }
    ...
    app.post('/listEpg', validSessionId, (req, res) => {
        ...
        if (!req.body.sessionId) {
            res.statusCode = 401;
            res.end();
        } else if (!req.body.sessionId || !req.body.channelId || !req.body.fromDate || !req.body.toDate) {
            res.statusCode = 400;
            res.end();
        } else {
    ...
    app.post('/deleteChannel',isDashboard, (req, res) => {
        req.db.channels.destroy({ where: { channelId: req.body.channelId } }).then((ok) => {
            res.statusCode = 200
            res.end()
        })
    })
    app.post('/updateChannel',isDashboard, (req, res) => {
        ...
        req.db.channels.update(channel, { where: { channelId: channel.channelId } }).then((ok) => {
            res.statusCode = 200
            res.end()
        })
    })
    app.post('/createChannel', isDashboard, (req, res) => {
        ...
        req.db.channels.create(newChannel).then(() => {
            res.statusCode = 201
            res.end()
    ...
    app.post('/deleteFromPackage',isDashboard, (req,res) => {
        if(!req.body.channel) {
            ...
            req.db.chPackages.destroy({where: { channelId: req.body.channel.channelId, packageId: req.body.channel.packageId}}).then(( )=> {
                res.statusCode = 200
                res.end()
            })
        }
    ...
    app.post('/sortChannels',isDashboard, (req,res) => {
        ...
        if(!req.body.channels) {
            ...
            req.body.channels.forEach(async (el) => {
                await req.db.chPackages.update({order: el.order}, { where: { channelId: el.channelId, packageId: el.packageId } })
            })
            res.statusCode = 200
            res.end()
        }
    ...
    function validSessionId(req, res, next) {
        if (!req.body.sessionId) {
            ...
            req.db.sessions.findOne({ where: { key: req.body.sessionId } }).then(ok => {
                if (!ok) {
                    res.statusCode = 401
                    res.end()
                } else {
